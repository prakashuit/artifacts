...
def plan_with_llm(user_prompt: str, services_info: str) -> str:
    """Use LLM to plan which services to call and construct URLs"""
...
def plan_with_llm_streaming(user_prompt: str, services_info: str):
    """Use LLM to plan with streaming"""
    system_prompt = f"""You are an API orchestration expert. Based on the user's request and available services, you must:
1. Identify which service(s) need to be called
2. Determine the order of execution (if multiple services)
3. Construct the exact URL(s) that would work in Postman
...
def execute_quick_mode_agentic(user_prompt: str, services_info: str):
    ...
    full_plan_response = ""
    for chunk in plan_with_llm_streaming(user_prompt, services_info):
    ...
    if st.session_state.execution_mode == "quick":
        ...
        if st.button("🚀 Execute (Automatic)", use_container_width=True, type="primary"):
            if not user_prompt:
                ...
                st.session_state.quick_mode_result = execute_quick_mode_agentic(user_prompt, services_info)
...
st.divider()
st.caption("🚀 Agentic API Orchestrator v2.0 | Powered by Claude AI")
