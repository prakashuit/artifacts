...
This document covers the end-to-end design: normalisation, price basis identification, feed notification handling, alert queuing, cache population, and the Research Agent MDP tool. It defines three DynamoDB tables (`falcon-market-data`, `falcon-operational`), a config store (`falcon-config` via AWS Parameter Store), TTLs, and the AWS service mapping.
...
- Alert hold queue (DynamoDB `falcon-operational` ALERTQUEUE entity) and release mechanism
- DynamoDB tables — `falcon-market-data` (prices, security master, corporate actions) and `falcon-operational` (pipeline state)
...
Normalisation Service (ECS Fargate)
  ...
  if priceType == PREV_DAY_CLOSE:
      → check falcon-market-data (PRICE entity, previous businessDate)
      → HIT:  enrich alert → MSK enriched topic → Research Agent
      → MISS: exception pull from MDP → write to falcon-market-data → enrich → Research Agent
      [never queue]

  if priceType == CURR_DAY_CLOSE or INTRADAY:
      → check falcon-operational: PK="{region}#{assetType}", SK="FEEDSTATE#{businessDate}"
      → COMPLETE: check falcon-market-data price cache
                  → HIT:  enrich → Research Agent
                  → MISS: exception pull → write to falcon-market-data → enrich → Research Agent
      → PENDING:  write to falcon-operational ALERTQUEUE
                  (PK="{region}#{assetType}", SK="ALERT#{businessDate}#{alertId}", status=PENDING)
...
MDP ready event arrives
  ...
  → Feed Handler (ECS Fargate)
      → idempotency: write falcon-operational FEEDSTATE = COMPLETE (conditional write)
                     PK="{region}#{assetType}", SK="FEEDSTATE#{businessDate}"
      → query falcon-operational: PK="{region}#{assetType}", SK begins_with "ALERT#{businessDate}#"
      → deduplicate instrumentIds across queued alerts
      → call MDP price API for all unique instruments
      → write prices to falcon-market-data PRICE entities
      → enrich each queued alert with fetched price
      → publish enriched alerts to MSK enriched topic
      → update each falcon-operational ALERTQUEUE item: status = RELEASED
...
Research Agent (ECS + Bedrock)
  ...
  → uses MDP Tool for additional data (comparison prices, security master, corporate actions)
      → check falcon-market-data → HIT: return | MISS: exception pull → write to falcon-market-data → return
...
**Output:** MSK `alert.enriched` topic (direct path) OR `falcon-operational` ALERTQUEUE write (hold path).
...
4. On exception pull: call MDP Integration Layer, write to `falcon-market-data`, proceed.
...
2. Idempotency guard (conditional write to `falcon-operational` FEEDSTATE).
3. Query `falcon-operational` ALERTQUEUE for all PENDING alerts for the feed cell.
4. Deduplicate instruments across queued alerts.
5. Batch call MDP via MDP Integration Layer.
6. Write prices to `falcon-market-data`.
...
```
1. Build falcon-market-data key from inputs
2. Query falcon-market-data
3. HIT  → return data to agent
4. MISS → call MDP Integration Layer
         → write to falcon-market-data with standard TTL
...
>
> **Fix:** DynamoDB `falcon-operational` table, with the feed cell as the partition key, is the authoritative, durable, queryable hold state. SQS is removed from the critical path.

| Job | Service |
|-----|---------|
| Hold state — who is waiting, for which feed cell | `falcon-operational` ALERTQUEUE entity |
| Feed cell completion state | `falcon-operational` FEEDSTATE entity |
| Delivery — enriched alerts to Research Agent | MSK `alert.enriched` topic |

### 6.2 ALERTQUEUE and FEEDSTATE Entities (`falcon-operational`)
...
|--------|----|----|------------|
| FEEDSTATE | `{region}#{assetType}` | `FEEDSTATE#{businessDate}` | `FEEDSTATE#2024-01-15` |
| ALERTQUEUE | `{region}#{assetType}` | `ALERT#{businessDate}#{alertId}` | `ALERT#2024-01-15#uuid-001` |
...
**Access patterns for `falcon-operational`:**
- **Check feed state:** `GetItem PK="{region}#{assetType}", SK="FEEDSTATE#{date}"`
- **Write feed state:** `UpdateItem PK="{region}#{assetType}", SK="FEEDSTATE#{date}"`
- **Enqueue alert:** `PutItem PK="{region}#{assetType}", SK="ALERT#{date}#{alertId}"`
- **Drain all alerts for a cell/date:** `Query PK="{region}#{assetType}", SK begins_with "ALERT#{date}#"`
- **Update alert status:** `UpdateItem PK="{region}#{assetType}", SK="ALERT#{date}#{alertId}"`

### 6.3 Enqueue Condition (Normalisation Service)

An alert is parked in `falcon-operational` ALERTQUEUE only when **all** of the following are true:
- `priceType ∈ { CURR_DAY_CLOSE, INTRADAY }`
- `falcon-operational` FEEDSTATE for `(region, assetType, businessDate)` is PENDING or absent

If FEEDSTATE is COMPLETE when the alert arrives, proceed directly to `falcon-market-data` price cache check — no queuing.

**Enqueue write:**
```
falcon-operational.put_item(
    PK           = "EMEA#CommonStock",           # region#assetType (feed cell)
    SK           = f"ALERT#2024-01-15#{alertId}", # ALERT#date#alertId
    status       = "PENDING",
...
```
falcon-operational.update_item(
    PK = "EMEA#CommonStock",      # region#assetType
    SK = "FEEDSTATE#2024-01-15",  # FEEDSTATE#date
...
```
items = falcon-operational.query(
    KeyConditionExpression = "PK = :pk AND begins_with(SK, :prefix)",
    ExpressionValues       = {
        ":pk":     "EMEA#CommonStock",    # region#assetType
        ":prefix": "ALERT#2024-01-15#"    # ALERT#date# prefix
    }
    FilterExpression = "status = :pending"
)
# Returns ALL pending alerts for this feed cell on this date.
# The unified PK design means one targeted query covers all alerts — no SQS polling,
# no visibility timeout management, no group-filter ambiguity.
```
...
**Step 5 — Write prices to `falcon-market-data`**
```
for (instrument, price) in prices.items():
    falcon-market-data.put_item(
...
```
for item in items:
    price = falcon-market-data.get_item(
    ...
    falcon-operational.update_item(
        PK         = "EMEA#CommonStock",      # region#assetType
        SK         = item.SK,                  # ALERT#2024-01-15#uuid
        SET status = "RELEASED",
        releasedAt = now()
    )
```
...
ALERTQUEUE items in `falcon-operational` remain PENDING until explicitly marked RELEASED. A Feed Handler crash at any step is fully recoverable:

```
Feed Handler restarts or receives duplicate notification:
→ Step 1: falcon-operational FEEDSTATE conditional write fails (already COMPLETE) → skip
→ Step 2: Query falcon-operational ALERT#2024-01-15# → returns only what has not yet been released
→ Steps 3–6: resume; MDP call is idempotent; falcon-market-data price write is conditional
...
| Scenario | Handling |
|----------|----------|
| MDP call fails (retries exhausted) | `status → FAILED` in `falcon-operational` ALERTQUEUE. Publish to MSK enriched with `priceAvailable: false`. CloudWatch alarm. |
...
| Feed Handler crashes mid-drain | On restart: `falcon-operational` still shows PENDING items. Query at Step 2 returns only unreleased alerts. Resume from Step 4. |
...
| Feed notification never arrives | `falcon-operational` ALERTQUEUE TTL auto-expires items after 24 h. EventBridge Scheduler queries for PENDING items older than feed SLA window and releases with `priceAvailable: false`. CloudWatch alarm. |
| Operations visibility | At any time: query `falcon-operational` PK="{region}#{assetType}", SK begins_with "ALERT#{date}" to see all PENDING, RELEASED, FAILED alerts for any feed cell. |

### 6.7 PREV_DAY_CLOSE Fast Path

```
1. priceType = PREV_DAY_CLOSE
2. falcon-market-data get: PK="PRICE#{instrumentId}#{region}#{assetType}" SK="PREV_DAY_CLOSE#{yesterday}"
3. HIT  → enrich → MSK alert.enriched → Research Agent
4. MISS → MDP exception pull → falcon-market-data put (TTL = 48 h) → enrich → publish
```

Never written to `falcon-operational`. Never waits for today's feed notification. This is the **exception pull** (sync service per security) for PREV_DAY_CLOSE.

### 6.8 Idempotency Summary

| Component | Mechanism |
|-----------|-----------|
| Feed Handler activation | `falcon-operational` conditional write on FEEDSTATE (`status != COMPLETE`) |
...
| `falcon-market-data` price write | Conditional on `attribute_not_exists(cachedAt)` unless explicit refresh |
...
|-------|----------|-----------|
| `falcon-market-data` | PRICE, SECMASTER, CORPACTION | MDP-sourced data; Research Agent accesses all three for the same instrument in one investigation; different PKs per entity type prevent partition competition |
| `falcon-operational` | ALERTQUEUE, FEEDSTATE | Feed cell as unified PK; co-owned by same two services; all transient (24–48 h TTL); querying alerts and feed state in one targeted query |
...
- **On-demand billing:** recommended for `falcon-market-data` (unpredictable write spikes during feed windows) and `falcon-operational` (bursty during drain)

---

### 7.2 Table: `falcon-market-data`
...
**Partition key (PK):** `{region}#{assetType}` — the feed cell (e.g. `EMEA#CommonStock`)
**Sort key (SK):** entity-type prefix + date
**TTL attribute:** `ttl`

The feed cell as PK is the key design insight: both FEEDSTATE and ALERTQUEUE are naturally grouped by feed cell. A single `Query PK="EMEA#CommonStock"` with a SK prefix filter returns exactly the data the Feed Handler needs without any cross-table joins or secondary indexes.
...
| Key | Pattern | Example |
|-----|---------|---------|
| PK | `{region}#{assetType}` | `EMEA#CommonStock` |
| SK | `FEEDSTATE#{businessDate}` | `FEEDSTATE#2024-01-15` |
...
**Read path:** Normalisation Service (is this feed cell complete before deciding to queue?).
...
|-----|---------|---------|
| PK | `{region}#{assetType}` | `EMEA#CommonStock` |
| SK | `ALERT#{businessDate}#{alertId}` | `ALERT#2024-01-15#uuid-001` |
...
**Read path:** Feed Handler drain query — `Query PK="EMEA#CommonStock", SK begins_with "ALERT#2024-01-15#"`.
...
**`falcon-market-data`:** No GSI required. All access patterns are point lookups or range scans on the main key. Add a GSI if operational reporting (e.g. "list all prices for a feed cell") becomes a requirement.

**`falcon-operational`:** No GSI required. The unified feed-cell PK design means all drain queries use the main table key efficiently. For the EventBridge fallback job that needs to find old PENDING alerts, it queries known feed cells (from the timing matrix config) rather than scanning the table.
...
| Table | Entity | TTL |
|-------|--------|-----|
| `falcon-market-data` | PRICE — PREV_DAY_CLOSE | businessDate + 48 h |
| `falcon-market-data` | PRICE — CURR_DAY_CLOSE | businessDate + 24 h |
| `falcon-market-data` | PRICE — INTRADAY | businessDate + 12 h |
| `falcon-market-data` | SECURITY_MASTER | cachedAt + 24 h |
| `falcon-market-data` | CORPORATE_ACTION | effectiveDate + 30 days |
| `falcon-operational` | FEEDSTATE | businessDate + 48 h |
| `falcon-operational` | ALERTQUEUE | queuedAt + 24 h |
...
falcon-market-data get: PK="PRICE#X#EMEA#CommonStock" SK="PREV_DAY_CLOSE#2024-01-14"
→ HIT:  enrich → MSK enriched topic → Research Agent       (~<50 ms)
→ MISS: MDP.getPrice(X, PREV_DAY_CLOSE, 2024-01-14)
        → falcon-market-data put (TTL = 48 h)
...
# Check feed state
falcon-operational get: PK="EMEA#CommonStock", SK="FEEDSTATE#2024-01-15" → status=PENDING

# Queue the alert
falcon-operational put:
    PK = "EMEA#CommonStock",            # region#assetType
    SK = f"ALERT#2024-01-15#{alertId}", # ALERT#date#alertId
    status = PENDING, instrumentId = X, alertPayload = { full alert }, ttl = now+24h

[alert waits durably in falcon-operational]

MDP publishes ready event: { region:EMEA, assetType:CommonStock, businessDate:2024-01-15 }
Feed Handler:
  1. falcon-operational update: PK="EMEA#CommonStock", SK="FEEDSTATE#2024-01-15"
     → status=COMPLETE (conditional write — idempotency guard)
  2. falcon-operational query: PK="EMEA#CommonStock", SK begins_with "ALERT#2024-01-15#"
     → returns all pending alerts [X, Y, Z, ...]
  3. uniqueInstruments = [X, Y, Z]
  4. MDP.batchGetPrices([X,Y,Z], CURR_DAY_CLOSE, 2024-01-15)
  5. falcon-market-data put: PRICE entities for X, Y, Z
  6. For each alert: enrich → MSK alert.enriched → Research Agent
  7. falcon-operational update each: SK="ALERT#2024-01-15#{alertId}", status=RELEASED
```

### 8.3 CURR_DAY_CLOSE — Feed Already Received

```
Alert arrives after falcon-operational FEEDSTATE = COMPLETE for that feed cell.

falcon-operational get: PK="EMEA#CommonStock", SK="FEEDSTATE#2024-01-15" → COMPLETE
falcon-market-data get: PK="PRICE#X#EMEA#CommonStock", SK="CURR_DAY_CLOSE#2024-01-15"
  HIT:  enrich → Research Agent
  MISS: MDP exception pull → falcon-market-data put → enrich → Research Agent
```
...
| Scenario | Handling |
|----------|----------|
| Feed notification arrives before any alerts | `falcon-operational` FEEDSTATE written COMPLETE. Subsequent alerts for that cell see COMPLETE and go to `falcon-market-data` cache check directly — no queuing. |
...
| PREV_DAY_CLOSE on Monday | businessDate = Friday. `falcon-market-data` cache check for Friday's close. Exception pull if not cached. |
| New security, never cached | `falcon-market-data` miss → exception pull → populate → proceed. |
| Feed never arrives | `falcon-operational` ALERTQUEUE TTL auto-expires after 24 h. EventBridge Scheduler queries known feed cells for PENDING items older than SLA window, releases with `priceAvailable: false`. CloudWatch alarm. |
| MDP API unavailable at drain time | Retry ×3 with backoff. After exhaustion: `falcon-operational` ALERTQUEUE status → FAILED, publish to MSK enriched with `priceAvailable: false`. Circuit breaker opens after threshold. |
| System restart mid-drain | `falcon-operational` ALERTQUEUE items remain PENDING. On restart, Feed Handler re-checks FEEDSTATE (COMPLETE), re-queries ALERT# items, resumes from Step 4. No message replay needed. |
| CURR_DAY_CLOSE alert on a non-trading day | No MDP notification will arrive. Alert stays PENDING in `falcon-operational` until TTL expiry, then EventBridge releases with `priceAvailable: false`. |
...
| Market data cache | Amazon DynamoDB — `falcon-market-data` | PRICE, SECMASTER, CORPACTION; on-demand billing; TTL-based expiry |
| Pipeline state | Amazon DynamoDB — `falcon-operational` | ALERTQUEUE + FEEDSTATE; unified feed-cell PK; all data transient, 24–48 h TTL |
| Price basis config | AWS Systems Manager Parameter Store | PRICEBASIS config; versioned; loaded in-memory at startup |
| Failure / expiry handling | Amazon EventBridge Scheduler + CloudWatch | Queries `falcon-operational` for PENDING items older than feed SLA; releases with priceAvailable=false |
